...
def redis_insert_without_join(iter, host, passwd, port, db):
    ...
    #     if redis_db.exists(tup[0]):
            ...
            redis_db.hincrby(tup[0], 'num',   1)
            redis_db.hincrby(tup[0], 'stars', tup[1][0])
            redis_db.hincrby(tup[0], 'words', tup[1][1])
        # else:
        #     redis_db.hmset(tup[0], {'num': tup[1], 'stars': tup[2], 'words': tup[3]} )
...
def main():
    ...
    for filename_tuple in [
        ...
        # using_databricks(filename_tuple, sc, True)
        without_databricks(filename_tuple, sc, True)
        # without_join(filename_tuple, sc, False)
